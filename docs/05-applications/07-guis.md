...
GNOME's built-in screen recorder is available via `Ctrl + Shift + Alt + R`. Recordings are saved as `.webm` to `~/Videos/`. Use the `webm2mp4` shell function to convert them to `.mp4`.
